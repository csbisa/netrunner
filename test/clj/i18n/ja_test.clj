...
(deftest turn-state
  (render-test {:username "Corp"
                ...
                :state {:phase "start-turn"
                        ...
                        :cards 5
                        :side "corp"}}
               "CorpはＨＱに5枚のカードと5 [Credits]でターン1目が開始する。")
  (render-test {:username "Corp"
                :type :turn-state
                :state {:phase "end-turn"
                        ...
                        :cards 0
                        :side "corp"}}
               "CorpはＨＱに0枚のカードと15 [Credits]でターン3目が終了する。")
  (render-test {:username "Runner"
                :type :turn-state
                :state {:phase "start-turn"
                        :turn 1
                        ...
                        :cards 5
                        :side "runner"}}
               "Runnerはグリップに5枚のカードと5 [Credits]でターン1目が開始する。")
  (render-test {:username "Runner"
                :type :turn-state
                :state {:phase "end-turn"
                        :turn 3
                        ...
                        :cards 0
                        :side "runner"}}
               "Runnerはグリップに0枚のカードと15 [Credits]でターン3目が終了する。"))
...
(deftest effect
  ...
  (render-test {:username "Runner" :type :use :card "foo" :effect {:reduce-str {:card "bar"}}}
               "Runner uses foo to give -1 strength to bar.")
  ...
  (render-test {:username "Runner" :type :use :card "foo" :effect {:bypass "bar"}}
  ...
  (render-test {:username "Runner" :type :use :card "Leech"
                :effect {:reduce-str {:card "Ice Wall" :server [:servers :hq] :pos 1}}}
               "Runner uses Leech to give -1 strength to Ice Wall protecting HQ at position 1.")
