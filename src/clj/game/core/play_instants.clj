...
(defn- complete-play-instant
  ...
  (system-msg state side (merge {:type :play :card title :ignore-cost ignore-cost}
                                (when-not ignore-cost {:cost payment-str})
                                (when as-flashback {:zone [:discard]})))
                          ...
                          (if (and c trash-after-resolving)
                            (let [trash-or-move (if (= zone :rfg) async-rfg trash)]
                              (wait-for (trash-or-move state side c {:unpreventable true})
                                        ...
                                        (when (= zone :rfg)
                                          (system-msg state side {:type :rfg :card (:title c)}))
